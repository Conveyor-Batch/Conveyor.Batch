---
title: "ADR-002: EF Core as the Job Repository Persistence Mechanism"
---
...
See [Repository API](/api/repository) for `EfCoreJobRepository` and `ConveyorBatchDbContext`. `Conveyor.Batch.Dapper` (see [Dapper](/api/dapper)) later added a Dapper-backed item *reader* as a lighter-weight alternative to `EfCoreItemReader` for data ingestion, but the job repository itself remains EF Core–only.

← [Back to ADR index](/adr/)
