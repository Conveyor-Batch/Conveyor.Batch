...
See [Repository API](/api/repository) for `EfCoreJobRepository` and `ConveyorBatchDbContext`, and [Dapper](/api/dapper) for the status of a possible future lighter-weight alternative.
