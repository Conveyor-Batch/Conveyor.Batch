---
title: "ADR-003: Polly v8 Adapter Pattern for Retry (Not a Hard Dependency)"
---
...
- A future `Conveyor.Batch.Polly` package will provide `PollyRetryPolicy : IRetryPolicy` wrapping `ResiliencePipeline`. It has not shipped yet — see [Skip & Retry](/guide/skip-retry) for the currently available options.
